...
def get_arguments():
    ...
    parser.add_argument('--data_path', dest='data_path', default="./toy",
    ...
    parser.add_argument("--save_path", type=str, default="./samples/output/test.pth",help="path to save trained Sound2Scene")
...
def validate(args, audio_encoder, generator, model, test_loader, device, epoch):
    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    criterion = InfoNCE_with_L2(device)

    total_loss = torch.Tensor([0])
    for step, (index, spec, emb_img, orig_img) in enumerate(test_loader):
        _, gt_aud_emb = audio_encoder(spec.unsqueeze(1).float().cuda())

        # _, img_emb = model(gt_img_emb)
        emb_img = Variable(emb_img.squeeze(1)).to(device)

        _, img_emb = model(emb_img)

        loss = criterion.loss_fn(gt_aud_emb,img_emb, index)
        total_loss += loss.item()

    writer.add_scalar("Loss/validation", total_loss.detach().item(), epoch)
...
def train(args):
    ...
    min_loss = torch.tensor(1000000).detach().cpu()
    ...
    for epoch in range(args.epochs):
        ...
        for step, (index, spec, emb_img, orig_img) in enumerate(train_loader):
            ...
            # gt_img_emb, _ = feature_extractor(emb_img.squeeze(1).cuda())
            ...
            # deep_features += gt_aud_emb.detach().cpu().numpy().tolist()

            deep_features += img_emb.detach().cpu().numpy().tolist()
            actual += index
            total_loss += loss.detach().item()
            total_similarity += cosine_similarity(gt_aud_emb.detach().cpu().numpy().reshape(1, -1), img_emb.detach().cpu().numpy().reshape(1, -1))
        ...
        for i, label in zip(range(len(cluster)), actual):
            plt.scatter(cluster[i, 0], cluster[i, 1], marker='.', label=label)

        filename = "samples/output/graph/graph" + "_" + str(epoch) + ".jpg"
        ...
        print(f'{epoch} : {total_similarity}')
        writer.add_scalar("Loss/train", total_loss, epoch)
        writer.add_scalar("Similarity/train", total_similarity, epoch)

        #validate
        model.eval()
        val_loss = validate(args, audio_encoder, generator, model, test_loader, device, epoch)
        model.train()
